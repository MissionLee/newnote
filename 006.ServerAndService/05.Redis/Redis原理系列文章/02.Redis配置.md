...
- [Redis 配置](#redis-配置)
  - [开头说明](#开头说明)
  - [INCLUDES](#includes)
    - 一份文件放通用配置，其他文件可以导入这份文件
  - [MODULES](#modules)
    - 引入模块
  - [NETWORK](#network)
    - 监听接口和TCP链接设置
  - [GENERAL](#general)
    - 一般配置，包括日志文件，打开的数据库数量，后台运行等
  ...
  - [MEMORY MANAGEMENT](#memory-management)
    - 内存策略，当内存用的很多的时候，用什么策略来清理
  - [LAZY FREEING](#lazy-freeing)
    - 懒释放：当一次进行大规模删除（例如清空）的时候，会造成redis阻塞，启用相关配置，优化执行
  - [APPEND ONLY MODE](#append-only-mode)
    - 通常redis周期性的把数据落到磁盘上，这里用AOF模式来持久化数据
  - [LUA SCRIPTING](#lua-scripting)
    - 限制lua脚本最大时间
  - [REDIS CLUSTER & CLUSTER DOCKER/NAT support](#redis-cluster)
    - redis集群
  - [SLOW LOG](#slow-log)
    - 慢日志 Slow log 是 Redis 用来记录查询执行时间的日志系统。
  - [LATENCY MONITOR](#latency-monitor)
    - 监控耗时操作
  - [EVENT NOTIFICATION](#event-notification)
    - 键空间通知使得客户端可以通过订阅频道或模式， 来接收那些以某种方式改动了 Redis 数据集的事件。
  - [ADVANCED CONFIG](#advanced-config)
    - 数据类型相关的一些设置：当hashes比较小的时候，使用一种更高效的编码，当hashes变得太大的时候，使用另外的-此处配置了其中的临界值
    - 还有一些其他的类似的配置
...
## REDIS CLUSTER &  CLUSTER DOCKER/NAT support
　　
```conf
################################ REDIS CLUSTER  ###############################
#
